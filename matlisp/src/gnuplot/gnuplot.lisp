...
(defun gnuplot-print (s a &rest args)
  (assert (every (let (d) #'(lambda (x) (= (dimensions x a) (or d (setq d (dimensions x a)))))) args) nil 'tensor-dimension-mismatch)
  (apply #'mapslicec~ a
	 #'(lambda (&rest vecs)
	      (map nil #'(lambda (v) (declare (type (and real-numeric-tensor base-vector) v))
			    (mapslicec~ 0 #'(lambda (y) (print-element y (ref y 0) s) (format s "~,4t")) v)) vecs)
	      (format s "~%"))
	 args)
  (format s "e~%"))
...
(defun plot (data &key (style "lines") (color #x0000ff))
  ...
  (labels ((ppair (s x y color style)
	     (let ((*tensor-print-parameters* `(t t 0)))
	       (format s "'-' using 1:2 with ~a linecolor rgb \"#~X\"~%" style color)
	       (print-tensor (apply #'join 1 (mapcar #'(lambda (x) (suptensor~ x 2)) (list x y))) s)
	       (format s "e e~%"))))
    (with-gnuplot-stream (s)
...
(defun splot (data)
  (let ((fname "/tmp/matlisp-gnuplot.out"))
    (with-open-file (s fname :direction :output :if-exists :supersede :if-does-not-exist :create)
      (loop :for i :from 0 :below (loop :for x :in data :minimizing (total-size x))
